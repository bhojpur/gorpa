...
const (
	...
	// contentHashKey is the key we use to hash source files. Change this key and you'll break all past versions of all Bhojpur GoRPA builds ever.
...
type Component struct {
	...
	// in its root. Otherwise this field is empty, in which case the application might still
...
// GitCommit returns the git commit of this component or the application. Returns an empty string if
// neither the component, nor the application are part of a working copy.
...
func (p *Package) findCycle() ([]string, error) {
	...
	walk = func(pkg *Package) ([]string, error) {
		...
		if len(path) <= depth {
			return nil, xerrors.Errorf("[internal error] depth exceeds max path length: looks like the application package index isn't build properly")
		}
...
func (v *PackageVariant) ResolveSources(application *Application, loc string) (incl []string, excl []string, err error) {
	incl, err = resolveSources(application, loc, v.Sources.Include, true)
	if err != nil {
		return
	}

	excl, err = resolveSources(application, loc, v.Sources.Exclude, true)
	if err != nil {
		return
	}
...
func resolveSources(application *Application, loc string, globs []string, includeDirs bool) (res []string, err error) {
	for _, glb := range globs {
		srcs, err := doublestar.Glob(loc, glb, application.ShouldIgnoreSource)
		if err != nil {
			return nil, err
		}
		...
		for _, src := range srcs {
			...
			if application.ShouldIgnoreSource(src) {
				continue
			}
			res = append(res, src)
		}
...
func (p *Package) FilesystemSafeName() string {
	...
	pkgdir = strings.Replace(pkgdir, ":", "--", -1)
	// components in the application root would otherwise start with - which breaks a lot of shell commands
